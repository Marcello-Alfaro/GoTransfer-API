...
export default {
  srcFileSent(srcEmail, dstEmail, dir) {
    return {
      to: srcEmail,
      from: FROM_EMAIL,
      subject: `"${dir.title}" was sent to ${dstEmail}`,
      html: `<!DOCTYPE html>
      <html
        lang="en"
                ...
                <td align="center" style="padding: 0">
                  ...
                  />
                        <h1
                          ...
                          "
                        >
                        Hi there, "${dir.title}" was sent successfully to ${dstEmail} with ${
  ...
  dstFileSent(srcEmail, dstEmail, dir) {
    return {
      to: dstEmail,
      from: FROM_EMAIL,
      subject: `${srcEmail} sent you "${dir.title}" with ${
        dir.Files.length > 1 ? `${dir.Files.length} files` : `1 file`
      }`,
      html: `<!DOCTYPE html>
      <html
                ...
                <td align="center" style="padding: 0">
                  ...
                  />
                        <h1
                          ...
                          "
                        >
                        Hi there, ${srcEmail} sent you "${dir.title}" with ${
        dir.Files.length > 1 ? `${dir.Files.length} files` : '1 file'
      }.
                        </h1>
                        <p>Total size: ${dir.size}</p>
                    ...
                    >
                      ...
                      />
                        ...
                        >
                          <ol>
                          ${dir.Files.reduce((accum, entry) => {
                            accum += `<li><a href="${API_URL}/files/download/${dir.dirId}/${entry.fileId}?isAuth=false&srcEmail=${srcEmail}&dstEmail=${dstEmail}">${entry.name}</a>
                            <p class="size">size: ${entry.size}</p>
                            </li>`;
                            return accum;
                          }, '')}
                          ...
                          ${
                            dir.Files.length > 1
                              ? `                                <a
                          href="${API_URL}/files/download/${dir.dirId}?isAuth=false&srcEmail=${srcEmail}&dstEmail=${dstEmail}"
                          style="
                            background: #0b7285;
                            text-decoration: none;
                            padding: 10px 25px;
