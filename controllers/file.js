  ...
  async postSendFile(req, res, next) {
    try {
      ...
      if (!isAuth) {
        ...
        const dir = await notAuthUser.createDir({ dirId, title, message, expire });
        ...
        return res.download(filepath, async (err) => {
          try {
            ...
            const msgToSource = {
              to: srcEmail,
              from: FROM_EMAIL,
              subject: `${dstEmail} downloaded one of your files`,
              html: `<!DOCTYPE html>
              <html
                lang="en"
                xmlns="http://www.w3.org/1999/xhtml"
          ...
          .on('finish', async () => {
            try {
              ...
              const msgToSource = {
                to: srcEmail,
                from: FROM_EMAIL,
                subject: `${dstEmail} downloaded all your files`,
                html: `<!DOCTYPE html>
                <html
                  lang="en"
                  xmlns="http://www.w3.org/1999/xhtml"
